...
class RealBattleArea {

	static WIDTH: any = 640;

	static HEIGHT: any = 480;
}

class RealBattle extends BaseBattle {

	_parentCoreAttack: any = null;

	_isBattleLayoutVisible: any = false;

	_isMotionBaseScroll: any = false;

	_idleCounter: any = null;

	_autoScroll: any = null;

	_uiBattleLayout: any = null;

	_battleArea: any = null;
	...
	setMotionBaseScroll(isMotionBaseScroll?) {
	...
	_setBattlerData(battler?, unit?, isSrc?, isRight?, versusType?) {
		var motionId, pos;
	...
	_getVersusType(unitSrc?, unitDest?) {
		var versusType;
	...
	_checkDamage(unit?, damage?, battler?) {
		var order = this._order;
...
class BaseBattler extends BaseObject {
	...
	_isPollingState: any = false;

	_pollingCounter: any = null;

	_isWaitForDamage: any = false;

	_realBattle: any = null;

	_isWaitLoopZone: any = false;

	_loopFrameIndex: any = 0;

	setupRealBattler(motionParam?, isSrc?, realBattle?) {
	...
	moveAvoidOrDamage() {
		...
		if (this._checkLoopZone()) {
			...
			if (order.nextOrder()) {
				...
				isAttack = motionCategoryType === MotionCategoryType.ATTACK || motionCategoryType === (MotionCategoryType as any).BOW || motionCategoryType === MotionCategoryType.MAGIC; // NOTE (JS->TS): BOW isn't a defined value in the original
	...
	drawBattler(xScroll?, yScroll?) {
		this._motion.drawMotion(xScroll, yScroll);
	}

	checkForceScroll(isContinuous?) {
		return false;
	}
	...
	getCenterPos(effectWidth?, effectHeight?) {
		var isRight = this._realBattle.getBattler(true) === this;
...
class DirectBattler extends BaseBattler {
...
class IndirectBattler extends BaseBattler {

	_isThrowState: any = false;
...
class MagicBattlerType {

	static NORMAL: any = 0;

	static INVOCATION: any = 1;
...
class MagicBattler extends BaseBattler {

	_invocationEffect: any = null;

	_magicEffect: any = null;
...
class MagicWeaponAttackBattler extends MagicBattler {
...
class RealEffect extends BaseCustomEffect {

	_motion: any = null;

	_isHitCheck: any = false;

	_realBattle: any = null;

	_isEasy: any = false;

	setupRealEffect(anime?, x?, y?, isRight?, realBattle?) {
		var motionParam;
	...
	drawEffect(xScroll?, yScroll?, isFront?) {
		var anime;
...
class RealAutoScroll extends BaseObject {
	...
	_isAutoStart: any = false;

	_isApproach: any = false;
...
class EnemyOffsetControl {

	static getOffsetPos(battler?) {
		var obj: any = {};
	...
	static _doOffsetAction(motionActive?) {
...
class UIBattleLayout extends BaseObject {

	// NOTE (JS->TS conversion): set/deleted externally via
	// AttackControl.getBattleObject().getUIBattleLayout()._disabledBackground (attack_realbattle.js
	// ~2508/2511), not through this. within this class - outside the reach of the assignment scan
	// used elsewhere in this conversion, so added by hand once tsc flagged the read at ~line 2277.
	_disabledBackground: any = undefined;
	...
	_statusRight: any = null;

	_statusLeft: any = null;

	_scrollBackground: any = null;

	_isMoveEnd: any = false;

	_battleContainer: any = null;

	setBattlerAndParent(battlerRight?, battlerLeft?, realBattle?) {
		var unit, targetUnit;
	...
	setDamage(battler?, damage?, isCritical?, isFinish?) {
		var gauge;
	...
	_drawBackground(xScroll?, yScroll?) {
		var pic;
	...
	_drawCustomPicture(xScroll?, yScroll?) {
	}

	_drawBattler(xScroll?, yScroll?, battler?, isRight?) {
		battler.drawBattler(xScroll, yScroll);
	}

	_drawEffect(xScroll?, yScroll?, isFront?) {
		var i, effect;
		var effectArray = this._realBattle.getEffectArray();
		var count = effectArray.length;
	...
	_getAttackStatus(unit?, targetUnit?, isSrc?) {
	...
	_showDamageAnime(battler?, isCritical?, isFinish?) {
	...
	_showDamagePopup(battler?, damage?, isCritical?) {
	...
	_isBackgroundDisabled() {
		return typeof this._disabledBackground !== 'undefined';
	}
}

class BaseBattleContainer extends BaseObject {

	_realBattle: any = null;

	setBattleObject(realBattle?) {
		this._realBattle = realBattle;
	}
...
class ClipingBattleContainer extends BaseBattleContainer {

	_battleClippingArea: any = null;

	_safeClipping: any = null;
...
class ScalingBattleContainer extends BaseBattleContainer {
...
class FixedFocusBattleContainer extends BaseBattleContainer {
