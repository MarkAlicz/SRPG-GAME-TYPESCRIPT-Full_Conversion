...
class AnimeSimple extends BaseObject {

	_animeData: any = null;

	_interpolationMode: any = 0;

	_parentAnimeMotion: any = null;

	_motionId: any = 0;

	_weaponResourceHandle: any = null;
	...
	setAnimeMotion(parentAnimeMotion?) {
	...
	setWeaponResourceHandle(weaponResourceHandle?) {
		this._weaponResourceHandle = weaponResourceHandle;
	}

	drawMotion(frameIndex?, i?, animeRenderParam?, animeCoordinates?) {
	...
	drawWeapon(frameIndex?, i?, animeRenderParam?, animeCoordinates?) {
	...
	_getMotionPicture(frameIndex?, i?, animeRenderParam?) {
	...
	_isColorChangeEnabled(frameIndex?, i?, animeRenderParam?) {
	...
	_getWeaponPicture(isShoot?, silhouetteType?) {
		var list, isRuntime;
		var base = root.getBaseData();
	...
	_checkReverseInfo(frameIndex?, i?, animeRenderParam?, animeCoordinates?): any {
		var isRight = true;
		var dx = 0;
		var dy = 0;
	...
	_isAlphaChangeEnabled(frameIndex?, i?, animeRenderParam?) {
		if (animeRenderParam === null) {
			return false;
		}
	...
	_drawSprite(x?, y?, width?, height?, pic?, isAbsolute?, isRight?, xSrc?, ySrc?, srcWidth?, srcHeight?, animeCoordinates?) {
...
class NonBattleAnimeSimple extends AnimeSimple {
...
class AnimeMotion extends BaseObject {
	...
	_rangeType: any = EffectRangeType.NONE;
	...
	_isThrowWeaponHidden: any = false;

	_isWeaponShown: any = true;

	_isLockSound: any = false;

	_includedResourceCount: any = 0;
	...
	_realEffect: any = null;

	setMotionParam(motionParam?) {
	...
	getEffectPos(effectAnimeData?, isRight?) {
		var dx, dy, offset;
		var index = this._getSpriteIndexFromSpriteType(SpriteType.KEY);
		var width = this._animeData.getSpriteWidth(this._motionId, this._frameIndex, index);
		var height = this._animeData.getSpriteHeight(this._motionId, this._frameIndex, index);
	...
	getCenterPos(effectWidth?, effectHeight?, isRight?) {
		var dx, dy, offset;
		var index = this._getSpriteIndexFromSpriteType(SpriteType.KEY);
		var width = this._animeData.getSpriteWidth(this._motionId, this._frameIndex, index);
		var height = this._animeData.getSpriteHeight(this._motionId, this._frameIndex, index);
	...
	setFrameIndex(frameIndex?, isFrameChange?) {
	...
	_getSpriteIndexFromSpriteType(spriteType?, frameIndex?) {
		var i;
		var count = this._animeData.getSpriteCount(this._motionId, this._frameIndex);
	...
	_isSpriteHidden(i?, spriteType?, motionCategoryType?) {
	...
	_isDualSprite(spriteIndex?, spriteType?, motionCategoryType?) {
	...
	_isBackgroundAnimeScaling() {
		return true;
	}

	_isBackgroundAnimeReverse() {
		return false;
	}
}
...
class AnimePerformanceHelper {

	static pickup(pic?, animeMotion?) {
		var profiler;
	...
	static _isLargeMemory(animeMotion?) {
