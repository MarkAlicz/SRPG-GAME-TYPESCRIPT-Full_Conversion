...
class TrophyCollector extends BaseObject {
	...
	_deleteTrophy(list?, trophy?) {
		...
		for (i = 0; i < count; i++) {
			data = list.getData(i);
			if (this._compareTrophy(data, trophy)) {
	...
	_compareTrophy(data?, trophy?) {
		var isGold = false;
		var isBonus = false;
		var isItem = false;
		var srcFlag = data.getFlag();
		var destFlag = trophy.getFlag();
		
		if ((srcFlag & TrophyFlag.GOLD) === (destFlag & TrophyFlag.GOLD)) {
			if ((srcFlag & TrophyFlag.GOLD) === 0) {
				isGold = true;
			}
			else if (data.getGold() === trophy.getGold()) {
				isGold = true;
			}
		}
		
		if ((srcFlag & TrophyFlag.BONUS) === (destFlag & TrophyFlag.BONUS)) {
			if ((srcFlag & TrophyFlag.BONUS) === 0) {
				isBonus = true;
			}
			else if (data.getBonus() === trophy.getBonus()) {
				isBonus = true;
			}
		}
		
		if ((srcFlag & TrophyFlag.ITEM) === (destFlag & TrophyFlag.ITEM)) {
			if ((srcFlag & TrophyFlag.ITEM) === 0) {
				isItem = true;
			}
			else if (ItemControl.compareItem(data.getItem(), trophy.getItem())) {
				isItem = true;
			}
		}
		
		return isGold && isBonus && isItem;
	}
}

class EventTrophyMode {
...
class EventTrophy extends BaseObject {
	...
	_completeMemberData(unit?, event?) {
		var trophy;
