...
class BaseTurnChange extends BaseObject {

	_straightFlow: any = null;

	_eventChecker: any = null;

	enterTurnChangeCycle() {
...
class TurnChangeStart extends BaseTurnChange {
...
class TurnChangeEnd extends BaseTurnChange {
...
class TurnChangeMapStart extends BaseTurnChange {
...
class BaseTurnLogoFlowEntry extends BaseFlowEntry {

	_turnChange: any = null;

	doMainAction(isMusic?) {
		var startEndType;
...
class TurnMarkFlowEntry extends BaseTurnLogoFlowEntry {

	_counter: any = null;

	enterFlowEntry(turnChange?) {
	...
	_prepareMemberData(turnChange?) {
		this._turnChange = turnChange;
		this._counter = createObject(CycleCounter);
	}

	_completeMemberData(turnChange?) {
...
class TurnAnimeFlowEntry extends BaseTurnLogoFlowEntry {
...
class RecoveryAllFlowEntry extends BaseFlowEntry {

	_dynamicEvent: any = null;

	_recentRecoverySkill: any = null;
	...
	_getRecoveryValueInternalForSkill(unit?) {
		var recoveryValue = 0;
	...
	_getRecoveryValueInternalForTerrain(unit?) {
		var recoveryValue = 0;
	...
	_getRecoveryValueInternalForState(unit?) {
		return StateControl.getHpValue(unit);
	}

	_arrangeValue(unit?, recoveryValue?) {
	...
	_isRecoveryAllowed(unit?, recoveryValue?) {
		return true;
	}

	_getTurnRecoveryAnime() {
		var anime;
...
class MetamorphozeCancelFlowEntry extends BaseFlowEntry {
...
class BerserkFlowEntry extends BaseFlowEntry {

	_berserkTurn: any = null;
...
class StateTurnFlowEntry extends BaseFlowEntry {
...
class ReinforcementAppearFlowEntry extends BaseFlowEntry {

	_reinforceChecker: any = null;
...
class ReinforcementCheckerMode {
...
class ReinforcementChecker extends BaseObject {

	_xScroll: any = 0;

	_yScroll: any = 0;

	_waitCounter: any = null;

	_reinforceUnitArray: any = null;

	_divisionAreaIndex: any = 0;

	_divisionAreaArray: any = null;

	enterReinforcementChecker(isSkipMode?) {
		this._prepareMemberData(isSkipMode);
		return this._completeMemberData(isSkipMode);
	}

	moveReinforcementChecker() {
		var mode = this.getCycleMode();
		var result = MoveResult.CONTINUE;
		
		if (mode === (ReinforcementCheckerMode as any).MOVE) { // NOTE (JS->TS): MOVE isn't a defined value (only TOP/WAIT) in the original
			result = this._moveTop();
		}
	...
	startMove() {
		...
		this._setMapScroll();
		this.changeCycleMode((ReinforcementCheckerMode as any).MOVE); // NOTE (JS->TS): same gap as above
	}

	_prepareMemberData(isSkipMode?) {
		this._xScroll = 0;
		this._yScroll = 0;
	...
	_completeMemberData(isSkipMode?) {
	...
	_checkReinforcementPos(divisionArea?) {
		var i, x, y, posData;
		var mapInfo = root.getCurrentSession().getCurrentMapInfo();
	...
	_checkReinforcementPage(posData?, arr?) {
	...
	_getTargetPos(posData?, pageData?) {
