...
class SimulateMove extends BaseObject {
	...
	_saveMostResentMov(unit?, moveCource?) {
		var i, direction;
		var count = moveCource.length;
		var n = 0;
		var x = unit.getMapX();
...
class CourceBuilder {
	...
	static createLongCource(unit?, goalIndex?, simulator?) {
		var cource;
	...
	static createExtendCource(unit?, goalIndex?, simulator?) {
		var cource;
		var moveCount = ParamBonus.getMov(unit);
		var indexArrayDisabled = [];
	...
	static getValidGoalIndex(unit?, goalIndex?, simulator?, moveAIType?) {
	...
	static _getBlockUnitArray(unit?, simulator?, simulatorMap?, goalIndex?) {
	...
	static _createCource(unit?, goalIndex?, simulator?, indexArrayDisabled?, moveMaxCount?, type?) {
	...
	static _getSideIndex(x?, y?, movePoint?, simulator?) {
	...
	static _getNewGoalIndex(unit?, simulator?, indexArrayDisabled?, moveMaxCount?) {
	...
	static _validCource(unit?, cource?, simulator?, moveCount?) {
	...
	static _reverseCource(moveCource?) {
		var tmp;
		var i = 0;
