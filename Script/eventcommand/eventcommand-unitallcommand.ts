...
class UnitAllCommandEventCommand extends BaseEventCommand {
	...
	_getListArray(filter?, eventCommandData?) {
		return FilterControl.getListArray(filter);
	}

	_appendGuestUnit(filter?, eventCommandData?) {
		var i, targetUnit;
	...
	_pushFlowEntriesSync(straightFlow?) {
	...
	_pushFlowEntriesAsync(straightFlow?) {
...
class AllSyncAnimeFlowEntry extends BaseFlowEntry {

	_dynamicAnime: any = null;

	enterFlowEntry(commandParent?) {
		var anime, pos;
...
class AllAsyncAnimeFlowEntry extends BaseFlowEntry {

	_commandParent: any = null;

	_index: any = 0;

	_animeArray: any = null;
...
class AllScrollFlowEntry extends BaseFlowEntry {
	...
	_isCommandScrollable(targetUnit?, command?, commandType?) {
		var result = false;
	...
	_isUnitRemoveScrollable(targetUnit?, command?) {
		return false;
	}

	_isUnitAssignScrollable(targetUnit?, command?) {
		return false;
	}

	_isItemChangeScrollable(targetUnit?, command?) {
		return false;
	}

	_isParameterChangeScrollable(targetUnit?, command?) {
		return false;
	}

	_isSkillChangeScrollable(targetUnit?, command?) {
		return false;
	}

	_isHpRecoveryScrollable(targetUnit?, command?) {
	...
	_isDamageHitScrollable(targetUnit?, command?) {
		if (command.isGraphicsSkip()) {
			return false;
		}
	...
	_isExperiencePlusScrollable(targetUnit?, command?) {
		return false;
	}

	_isClassChangeScrollable(targetUnit?, command?) {
		if (command.isGraphicsSkip()) {
			return false;
		}
	...
	_isUnitStateChangeScrollable(targetUnit?, command?) {
		return false;
	}

	_isTrophyChangeScrollable(targetUnit?, command?) {
		return false;
	}

	_isItemDurabilityChangeScrollable(targetUnit?, command?) {
		return false;
	}

	_isUnitStateAdditionScrollable(targetUnit?, command?) {
	...
	_isUnitCapacityChange(targetUnit?, command?) {
		return false;
	}

	_isUnitMetamorphozeScrollable(targetUnit?, command?) {
		var anime;
	...
	_isMapPosOperationScrollable(targetUnit?, command?) {
		return false;
	}
}

class AllEventFlowEntry extends BaseFlowEntry {
	...
	_setEventCommand(generator?, targetUnit?, command?, commandType?) {
		if (commandType === EventCommandType.UNITREMOVE) {
	...
	_damagehit(generator?, targetUnit?, command?) {
	...
	_experiencePlus(generator?, targetUnit?, command?) {
	...
	_unitStateAddition(generator?, targetUnit?, command?) {
	...
	_unitCapacityChange(generator?, targetUnit?, command?) {
		generator.unitCapacityChange(targetUnit, -1);
	}

	_unitMetamorphoze(generator?, targetUnit?, command?) {
	...
	_mapPosOperation(generator?, targetUnit?, command?) {
	...
	_scriptExecute(generator?, targetUnit?, command?) {
		generator.scriptExecuteCode(targetUnit, '');
	}

	_isCommandSkip(command?) {
