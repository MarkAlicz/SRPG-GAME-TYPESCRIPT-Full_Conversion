...
class StateControl {
	...
	static isBadStateOption(unit?, option?) {
		var i, state;
	...
	static isBadStateFlag(unit?, flag?) {
		var i, state;
		var list = unit.getTurnStateList();
		var count = list.getCount();
	...
	static getTurnState(unit?, state?) {
		var i, turnState;
		var list = unit.getTurnStateList();
		var count = list.getCount();
	...
	static checkStateInvocation(active?, passive?, obj?) {
	...
	static arrangeState(unit?, state?, increaseType?) {
		var turnState = null;
		var list = unit.getTurnStateList();
		var count = list.getCount();
	...
	static getStateParameter(unit?, index?) {
		var i;
		var list = unit.getTurnStateList();
		var count = list.getCount();
		var value = 0;
	...
	static isStateBlocked(unit?, targetUnit?, state?) {
	...
	static isStateRecoverable(unit?, stateGroup?) {
		var i, state;
		var list = unit.getTurnStateList();
		var count = list.getCount();
	...
	static isStateGroupEnabled(state?, stateGroup?) {
		var i, count, refList;
	...
	static isTargetControllable(unit?) {
		var i, state, option;
		var list = unit.getTurnStateList();
		var count = list.getCount();
	...
	static _IsGuardStateCheckEnabled(unit?, targetUnit?, state?, item?) {
