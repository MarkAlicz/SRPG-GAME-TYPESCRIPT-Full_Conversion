...
class ItemControl {
	...
	static decreaseLimit(unit?, item?) {
		var limit;
	...
	static setEquippedWeapon(unit?, targetItem?) {
		var i, item;
		var count = UnitItemControl.getPossessionItemCount(unit);
	...
	static updatePossessionItem(unit?) {
		var mhp, scene;
	...
	static isWeaponAvailable(unit?, item?) {
		if (item === null) {
			return false;
		}
	...
	static isEffectiveData(unit?, item?) {
	...
	static compareItem(item?, targetItem?) {
		var id, targetId;
	...
	static _isItemTypeAllowed(unit?, item?) {
...
class UnitItemControl {

	static getItem(unit?, index?) {
		return unit.getItem(index);
	}

	static setItem(unit?, index?, item?) {
	...
	static getPossessionItemCount(unit?) {
		var i;
	...
	static getIndexFromItem(unit?, targetItem?) {
		var i, item;
		var count = UnitItemControl.getPossessionItemCount(unit);
...
class StockItemControl {

	static getStockItemArray() {
		return root.getMetaSession().getStockItemArray();
	}

	static getStockItem(index?) {
		var itemArray = this.getStockItemArray();
		return itemArray[index];
	}

	static setStockItem(index?, item?) {
		var itemArray = this.getStockItemArray();
	...
	static getStockItemCount() {
		var itemArray = this.getStockItemArray();
		return itemArray.length;
	}

	static isStockItemSpace() {
		return this.getStockItemCount() !== DataConfig.getMaxStockItemCount();
	}

	static getMatchItem(targetItem?) {
		var i, item;
...
class ItemChangeControl {

	static changeStockItem(targetItem?, increaseType?) {
		var arr = [];
	...
	static changeUnitItem(unit?, targetItem?, increaseType?, isStockSend?) {
		var arr = [];
	...
	static _increaseStockItem(targetItem?) {
		var arr = [];
	...
	static _decreaseStockItem(targetItem?) {
		var i, item;
	...
	static _increaseUnitItem(unit?, targetItem?) {
		var arr = [];
	...
	static _decreaseUnitItem(unit?, targetItem?, isStockSend?) {
		var i, item, curItem;
		var count = DataConfig.getMaxUnitItemCount();
		var arr = [];
	...
	static _releaseAllUnitItem(unit?, isStockSend?) {
		var i, curItem;
		var count = DataConfig.getMaxUnitItemCount();
		var arr = [];
